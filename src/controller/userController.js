...
exports.register = async function(req, res) {
  try {
    ...
    let { files } = req;
    if (!files?.length) {
      return res.status(400).send({ status: false, msg: 'profileimage  is missing' });
    }
    ...
    if (!regexName.test(fname))
      return res.status(400).send({ status: false, message: 'please enter valid characters' });
    if (!isValid(lname))
      return res.status(400).send({ status: false, message: 'lname cannot be empty' });
    if (!regexName.test(lname))
      return res.status(400).send({status: false,message: 'please enter valid lname characters'});
    if (!isValid(password))
      return res.status(400).send({ status: false, message: 'password cannot be empty' });
    if (!regexPassword.test(password))
      return res.status(400).send({status: false,message:'password must be between 8 to 15 digits one number and one alphabet'});
    //hashing password
    ...
    if (!regexEmail.test(email))
      return res.status(400).send({status: false,message: 'please enter valid email characters'});
    ...
    if (foundPhone)
      return res.status(400).send({ status: false, message: 'This phone is already being used' });
     address=JSON.parse(address)
    if (!isValid(address.shipping.street))
      return res.status(400).send({ status: false, message: 'street of shipping cannot be empty' });
    if (!isValid(address.shipping.city))
      return res.status(400).send({ status: false, message: 'city of shipping cannot be empty' });
    if (!isValid(address.shipping.pincode))
      return res.status(400).send({status: false,
        message: 'pincode of shipping  cannot be empty'});
    if (!regexPinCode.test(address.shipping.pincode))
      return res.status(400).send({status: false,message: 'please use valid pincode of shipping'});
    if (!isValid(address.billing.street))
      return res.status(400).send({ status: false, message: 'street of billing cannot be empty' });
    if (!isValid(address.billing.city))
      return res.status(400).send({ status: false, message: 'city of billing cannot be empty' });
    if (!isValid(address.billing.pincode))
      return res.status(400).send({ status: false, message: 'pincode of billing cannot be empty' });
    if (!regexPinCode.test(address.billing.pincode))
      return res.status(400).send({status: false,message: 'please use valid pincode of billing'});
...
exports.userLogin = async function(req, res) {
  try {
    ...
    let UsersData = await userModel.findOne({email});
    ...
    if (!UsersData || !unmasked) {
      return res
        .status(401).send({ status: false, msg: 'Enter a valid Email or Password' });
    }
    ...
    res.status(200).send({status: true,message: 'User Login Succesful',data: { userId: UsersData._id, token: token }});
  } catch (err) {
    // console.log(err);
  ...
  try {
    ...
    if (password) {
      if (!isValid(password))
        return res.status(400).send({ status: false, message: 'password cannot be empty' });
      if (!regexPassword.test(password))
        return res.status(400).send({status: false,message:'password must be between 8 to 15 digits one numbbe and one alphabet'});
      //hashing password
    ...
    if (phone) {
      if (!isValid(phone))
        return res.status(400).send({ status: false, message: 'phone cannot be empty' });
      if (!regexNumber.test(phone))
        return res.status(400).send({status: false,message: 'please enter valid phone characters'});
      const foundPhone = await userModel.findOne({ phone });
      if (foundPhone)
        return res.status(400).send({ status: false, message: 'This phone is already being used' });
    }
    if (address) {
      ...
      if (shipping) {
        ...
        if (street) {
          if (!isValid(street))
            return res.status(400).send({status: false,message: 'Shipping street   must be alphabetic characters'});
          savedObj['address.shipping.street'] = street;
        }
        if (city) {
          if (!isValid(city))
            return res.status(400).send({status: false,message: 'Shipping city must be alphabetic characters'});
          savedObj['address.shipping.city'] = city;
        }
        if (pincode) {
          if (!/^[0-9]{6}$/.test(pincode))
            return res.status(400).send({status: false,message: 'Shipping pincode must be number min length 6'});
      ...
      if (billing) {
        ...
        if (street) {
          if (!isValid(street))
            return res.status(400).send({status: false,message: 'Billing street   must be alphabetic characters'});
          savedObj['address.billing.street'] = street;
        }
        if (city) {
          if (!isValid(city))
            return res.status(400).send({status: false,message: 'Billing city must be alphabetic characters'});
          savedObj['address.billing.city'] = city;
        }
        if (pincode) {
          if (!/^[0-9]{6}$/.test(pincode))
            return res.status(400).send({status: false,message: 'Billing pincode must be number min length 6'});
    ...
    const updatedData = await userModel.findOneAndUpdate({ _id: userId }, {$set: savedObj},{ new: true });
