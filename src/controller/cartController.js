...
exports.getCart = async (req, res) =>{
  try {
    ...
    if (!userDoc) {return res.status(400).send({status: false,message: "user does not exist",});}
    ...
    try {
      ...
      if (req.params.cartId) {
        if (!cartId)
          return res
            .status(404)
            .send({ status: false, msg: "Please provide valid cartId" });
      }
      ...
      if (!checkCart)
       ...
       let deleteData = await cartModel.findOneAndUpdate(
        {_id:cartId},{isDeleted:true, deletedAt:Date.now()},{new:true})
